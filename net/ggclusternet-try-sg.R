...
binSG<-read.table("cytoscape.binSG.txt",header = T,sep="\t")
binSG$mean<-rowMeans(binSG,na.rm=T)
...
result2 = model_maptreeLJ(cor = corSG,
...
edge = edgeBuild(cor = corSG,node = node)#获得边的坐标
...
write.table(edge2,"networksg.txt",sep="\t",quote=F)

save.image("netsg.Rdata")
