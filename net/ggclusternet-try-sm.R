...
binSM<-read.table("cytoscape.binSM.txt",header = T,sep="\t")
binSM$mean<-rowMeans(binSM,na.rm=T)
...
result2 = model_maptreeLJ(cor = corSM,
...
edge = edgeBuild(cor = corSM,node = node)#获得边的坐标
...
write.table(edge2,"networksm.txt",sep="\t",quote=F)

save.image("netsm.Rdata")
