...
#匹配海域
data<-read.table("Blue carbon marine match.txt",header=T,sep="\t",quote="")
data<-data[,-c(3,4)]
RADdata <- merge(RADdata, data, by = "ID")#匹配样本数量的对话框
RADdata<-RADdata[,-1]
Geop.Chem<-RADdata[c("Marine","EC","TN", "pH", "C.N", "P", "C.P", "Fed", "Feo", "Fep","Ald","Alo","Alp")]
Org.C<-RADdata[c("Marine","TOC","DOC", "POC", "MOC", "Fe.OC", "Ca.OC", "Lignin.phenol", "Microbial.necromass")]
Others<-RADdata[c("Marine","Lon","Lat", "MAT", "MAP", "人口密度", "总种植面积")]
#Geop.Chem
RADdata_median <- as.data.frame(Geop.Chem %>% group_by(Marine) %>% summarize_if(is.numeric, list("q25" = ~quantile(., 0.25,type=2),"q75" = ~quantile(., 0.75,type=2),"q50" = ~quantile(., 0.50,type=2))))
median_only <- as.data.frame(Geop.Chem %>% group_by(Marine) %>% summarize_if(is.numeric, list("q50" = ~quantile(., 0.50,type=2))))
#Org.C
RADdata_median <- as.data.frame(Org.C %>% group_by(Marine) %>% summarize_if(is.numeric, list("q25" = ~quantile(., 0.25,type=2,na.rm = TRUE),"q75" = ~quantile(., 0.75,type=2,na.rm = TRUE),"q50" = ~quantile(., 0.50,type=2,na.rm = TRUE))))
median_only <- as.data.frame(Org.C %>% group_by(Marine) %>% summarize_if(is.numeric, list("q50" = ~quantile(., 0.50,type=2,na.rm = TRUE))))
#Others
RADdata_median <- as.data.frame(Others %>% group_by(Marine) %>% summarize_if(is.numeric, list("q25" = ~quantile(., 0.25,type=2,na.rm = TRUE),"q75" = ~quantile(., 0.75,type=2,na.rm = TRUE),"q50" = ~quantile(., 0.50,type=2,na.rm = TRUE))))
median_only <- as.data.frame(Others %>% group_by(Marine) %>% summarize_if(is.numeric, list("q50" = ~quantile(., 0.50,type=2,na.rm = TRUE))))

#Geop.Chem 13列
process_Marine_data <- function(data, Marine_name) {
  # 选择一个区域
  metadata_site <- subset(data, Marine == Marine_name)
...
#Others 7列
## 生成和保存蜘蛛图的函数
generate_and_save_spider_plot <- function(data, Marine_name) {
  ...
  # 创建PDF设备
  pdf(paste0(Marine_name, "_spider_plot.pdf"), width = 13, height = 13)
  # 打印图表
  print(gg)
  # 关闭PDF设备
  dev.off()
...
## 计算各站点数据框
BHS_data <- process_Marine_data(RADdata_median, "BHS")
ECS_data <- process_Marine_data(RADdata_median, "ECS")
SCS_data <- process_Marine_data(RADdata_median, "SCS")
YS_data <- process_Marine_data(RADdata_median, "YS")


## Generate and save the plots
generate_and_save_spider_plot(BHS_data, "BHS")
generate_and_save_spider_plot(ECS_data, "ECS")
generate_and_save_spider_plot(SCS_data, "SCS")
generate_and_save_spider_plot(YS_data, "YS")
...
rownames(bind_columns) <- bind_columns$Marine
bind_columns$Marine <- NULL

## 定义颜色和标题
colors <- c("#ec3123","#25377f", "#af8fd0","#f6bd21")
titles <- c("BHS","ECS","SCS","YS")
